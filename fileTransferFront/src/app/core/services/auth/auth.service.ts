...
export class AuthService {
  ...
  public login(username: string, password: string, remember: boolean): Observable<boolean> {
    ...
    const p = new Promise((resolve, reject) => {
      client
        .fetch(`${this.config.authBaseUrl}${LOGIN_PATH}`, {
          method: 'POST',
    ...
    return from(p).pipe(
      map(r => {
        ...
        if (response.error) {
          this.forbidden(response as ErrorResponse);
          return false;
        }
  ...
  public getRoles(): string[] {
    const data = this.storage.getData();
    return data ? data.datosLogin.roles : [];
  }

  public logout(): Observable<void> {
    return this.http
      .post(`${this.config.authBaseUrl}${LOGOUT_PATH}`, undefined)
      .pipe(
        mapTo(undefined),
        finalize(() => {
          //this.masterTablesService.removeStoreCache();
          //this.masterTablesService.removeShipperCache();
          this.storage.removeData();

          this.notification.warn('LOGOUT_SUCCESS');
        })
      );
  }
  ...
  public refreshToken(): Observable<boolean> {
    ...
    return this.http
      .post<LoginResponse>(
        `${this.config.authBaseUrl}${REFRESH_TOKEN_PATH}`,
        undefined,
        {
          headers: {
  ...
  public isTokenExpired(tokenData: TokenData): boolean {
    ...
    const expireDate = new Date(
      tokenDate.getTime() + tokenData.expirationMin * 60000
    );
    const now = new Date();
