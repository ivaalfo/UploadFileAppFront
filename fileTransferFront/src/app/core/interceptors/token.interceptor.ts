...
export class TokenInterceptor implements HttpInterceptor {
    ...
    return next.handle(request).pipe(
      catchError(error => {
        if (error instanceof HttpErrorResponse && error.status === 401) {
          ...
          this.notification.error('ERROR.DUPLICATED_KEY');
        } else if (
  ...
  private needsToken(request: HttpRequest<any>): boolean {
    const refreshTokenUrl =
      this.config.authBaseUrl + this.auth.getRefreshTokenPath();
    return (
      request.url.indexOf(this.config.apiBaseUrl) === 0 ||
      (request.url.indexOf(this.config.authBaseUrl) === 0 &&
