...
@Component({
  selector: 'm-reject-cmr-form',
  templateUrl: './reject-cmr-form.component.html'
})
export class RejectCMRFormComponent implements OnInit, OnDestroy {
  ...
  public constructor (
    ...
    this.pedidoForm = this.formBuilder.group({
      motivoRechazo: ['', [Validators.required, Validators.maxLength(100), Validators.pattern('[a-zA-Z0-9 çÇñÑáÁéÉíÍóÓúÚ@,(-_!)\&\/]*')]]
    });
  }
  ...
  public ngOnInit(): void {
    ...
    this.openerSubscription = this.opener$.subscribe(_ => {
      ...
      //Bloqueo el CMR para rechazo por USERNI
      if(this.directRJCT){
        this.lockForm(true);
        this.putPedidoLock();
      } else {
  ...
  public ngOnDestroy(): void {
    if (this.openerSubscription) {
      ...
      //Desbloqueo el CMR para rechazo por USERNI
      this.lockForm(false);
      this.done.emit();
    }
  }
  ...
  public cancelRej(unlock?: boolean): void {
    if(unlock) {
      this.lockForm(false);
  ...
  public accept(): void {
    ...
    if (this.pedidoForm) {
      if (this.pedidoForm.valid) {
        const motRechazo = this.pedidoForm.value.motivoRechazo ? this.pedidoForm.value.motivoRechazo : '';
        this.pedido.motivoRechazo = motRechazo;
        this.rejectCMRaction(this.pedido);
      }
    }
  }
  ...
  private initMotiveForm() {
    if(this.pedidoForm) {
      ...
      const rejectControl = this.pedidoForm.get('motivoRechazo');
  ...
  public rejectCMRaction(_pedido: PedidoProveedor): void {
    this.isLoading = true;
    this.apiValidadorClient.rejectCMR(LockEntities.LOCK_VALIDATOR, _pedido.track, _pedido.expediente, _pedido.motivoRechazo)
    .pipe(take(1))
    .subscribe((response: boolean) => {
      if (response) {
        this.onRejectActionSuccess();
      } else {
        const errorTitle = this.translate.instant("PEDIDOS_VAL.FORM.ERROR.CMR_REJECTION_ERROR", {refped: _pedido.track});
  ...
  private onRejectActionSuccess() {
    ...
    if(this.pedido) {
      //cmr rechazado
      const rejectedCMR = this.translate.instant("PEDIDOS_VAL.FORM.SUCCES.CMR_REJECTED", {refped: this.pedido.track});
      this.notification.warn(rejectedCMR, true, true);
    }
    this.onActionFinalize(true);
  }
