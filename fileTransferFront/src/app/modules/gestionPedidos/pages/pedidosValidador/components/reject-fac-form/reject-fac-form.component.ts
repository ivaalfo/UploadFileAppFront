...
export class RejectFACFormComponent implements OnInit, OnDestroy {
  ...
  public rejectFACaction(_pedido: PedidoProveedor): void {
    ...
    .subscribe((response: boolean) => {
      if (response) {
        ...
        const errorTitle = this.translate.instant("PEDIDOS_VAL.FORM.ERROR.FAC_REJECTION_ERROR", {refped: _pedido.track});
  ...
  private onRejectActionSuccess() {
    ...
    if(this.pedido) {
      //fac rechazada
      const rejectedFAC = this.translate.instant("PEDIDOS_VAL.FORM.SUCCES.FAC_REJECTED", {refped: this.pedido.track});
