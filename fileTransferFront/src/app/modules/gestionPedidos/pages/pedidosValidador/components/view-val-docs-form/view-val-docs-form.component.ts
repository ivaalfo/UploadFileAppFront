...
export class ViewValDocsFormComponent implements OnInit, OnDestroy {
  ...
  private getCMR(rutaCMR: string) {
    ...
    this.viewCMRheaderText = this.translate.instant('PEDIDOS_VAL.FORM.FILENAME', {filename: this.filename});
  ...
  public onValidateCMR(): void {
    ...
    .subscribe((response: ApiResponseWithData) => {
      if (response && (!response.errores || response.errores.length === 0)) {
        ...
        const warnTitle = this.translate.instant("PEDIDOS_VAL.FORM.ERROR.CMR_VALIDATION_ERROR", {refped: this.pedidoSelected.track});
      ...
      console.error(error);
      const errorTitle = this.translate.instant('PEDIDOS_VAL.FORM.ERROR.CMR_VALIDATION_ERROR', {refped: this.pedidoSelected.track});
  ...
  public onRejectCMR(pedido: PedidoProveedor): void {
    ...
    this.rejectTitle = this.translate.instant('PEDIDOS_VAL.FORM.ACTIONS.REJECT.CMR_TITLE', {refped: this.pedidoSelected.track});
    setTimeout(() => {
  ...
  public onAnotaCMR(pedido: PedidoProveedor): void {
    this.pedidoSelected = pedido;
    this.anotaTitle = this.translate.instant('PEDIDOS_VAL.FORM.ACTIONS.ANOTATION.TITLE', {refped: this.pedidoSelected.track});
    setTimeout(() => {
  ...
  public onRejectFAC(pedido: PedidoProveedor): void {
    this.pedidoSelected = pedido;
    this.rejectFACtitle = this.translate.instant('PEDIDOS_VAL.FORM.ACTIONS.REJECT.FAC_TITLE', {refped: this.pedidoSelected.track});
    setTimeout(() => {
  ...
  private onActionSuccess() {
    ...
    if (this.filename.length > 0) {
      //cmr actualizado
      const successCMR = this.translate.instant("PEDIDOS_VAL.FORM.SUCCES.CMR_VALIDATED", {refped: this.pedidoSelected.track});
