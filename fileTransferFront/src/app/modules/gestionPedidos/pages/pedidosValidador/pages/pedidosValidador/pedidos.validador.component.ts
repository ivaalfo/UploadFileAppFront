...
export class PedidosValidadorComponent implements OnInit {
  ...
  public getTableHeaders() {
    return [
      { title: 'PEDIDOS_VAL.TABLE.PROVEEDOR', sorted: true, property: 'proveedor'},
      { title: 'PEDIDOS_VAL.TABLE.USUPROV', sorted: true, property: 'proveedorUser'},
      { title: 'PEDIDOS_VAL.TABLE.TR', sorted: true, property: 'track'},
      { title: 'PEDIDOS_VAL.TABLE.EXPEDIENTE', sorted: true, property: 'expediente'},
      { title: 'PEDIDOS_VAL.TABLE.F_CARGA', sorted: true, property: 'fechaCarga'},
      { title: 'PEDIDOS_VAL.TABLE.REF_CARGA', sorted: true, property: 'refCarga'},
      { title: 'PEDIDOS_VAL.TABLE.F_DESCARGA', sorted: true, property: 'fechaDescarga'},
      { title: 'PEDIDOS_VAL.TABLE.R_DESCARGA', sorted: true, property: 'refDescarga'},
      { title: 'PEDIDOS_VAL.TABLE.ORIGEN', sorted: true, property: 'origen'},
      { title: 'PEDIDOS_VAL.TABLE.DESTINO', sorted: true, property: 'destino'},
      { title: 'PEDIDOS_VAL.TABLE.PALETS', sorted: true, property: 'numPalets'},
      { title: 'PEDIDOS_VAL.TABLE.CAMION', sorted: true, property: 'matriculaCamion'},
      { title: 'PEDIDOS_VAL.TABLE.REMOLQUE', sorted: true, property: 'matriculaRemolque'},
      { title: 'PEDIDOS_VAL.TABLE.F_SUBIDA_CMR', sorted: true, property: 'fechaSubidaCMRshort'},
      { title: 'PEDIDOS_VAL.TABLE.ANOTACIONES' }
    ];
  }
  ...
  public navigateToActivos(): void {
    this.router.navigate(['/gestionPedidos/pedidosActivos']);
  }

  public navigateToHistorico(): void {
    this.router.navigate(['/gestionPedidos/pedidosHistorico']);
  }
  ...
  public onRejectCMRselected(pedido: PedidoProveedor): void {
    this.selectedPedido = pedido;
    this.rejectTitle = this.translate.instant('PEDIDOS_VAL.FORM.ACTIONS.REJECT.CMR_TITLE', { refped: this.selectedPedido.track });
    setTimeout(() => {
  ...
  public onAnotaCMRselected(pedido: PedidoProveedor): void {
    this.selectedPedido = pedido;
    this.anotaTitle = this.translate.instant('PEDIDOS_VAL.FORM.ACTIONS.ANOTATION.TITLE', { refped: this.selectedPedido.track });
    setTimeout(() => {
  ...
  private selectPedido(pedido: PedidoProveedor) {
    ...
    if(this.selectedPedido.hasCMR===7){
      this.noAplica = this.translate.instant("PEDIDOS_VAL.TEXT.NO_APLICA");
    } else if(this.selectedPedido.hasCMR===2){
      this.noAplica = this.translate.instant("PEDIDOS_VAL.TEXT.YA_VALIDADO");
    }
  }
