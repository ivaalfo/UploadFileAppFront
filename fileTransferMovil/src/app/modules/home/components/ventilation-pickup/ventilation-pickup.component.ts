...
@Component({
  selector: 'm-ventilation-pickup',
  templateUrl: './ventilation-pickup.component.html',
  styleUrls: ['../task.component.scss']
})
export class VentilationPickupComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  public get containerNumber(): string {
    return this.containerTitle ? this.containerTitle : '';
  }

  protected onConfirmed(): void {
    this.spinnerService.show();
    if (this.selectedLicensePlate && this.transport) {
      this.actionSubscription = this.actionService.ventilationPickUp(this.transport)
        .pipe(
          take(1))
        .subscribe(response => {
          if (response) {
  ...
  private goToNext(): void {
    ...
    if (this.selectedLicensePlate) {
      this.router.navigate([`store-entrance/plate-number/${this.selectedLicensePlate}/${this.transport.transporteNi}`], {
        state: {
