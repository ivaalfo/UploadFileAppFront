...
import { TransportPoint } from '../../../../data/transport-point';
import { GlobalSpinnerService } from 'src/app/services/global-spinner/global-spinner.service';

@Component({
  selector: 'm-terminal-exit',
  templateUrl: './terminal-exit.component.html',
  styleUrls: ['../task.component.scss']
})
export class TerminalExitComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  protected onConfirmed(): void {
    ...
    if (this.selectedLicensePlate && this.transport) {
      this.actionSubscription = this.actionService.terminalExit(this.transport)
      .pipe(
        take(1))
      .subscribe(response => {
        if (response) {
  ...
  public goToNext(): void {
    ...
    if (this.selectedLicensePlate && this.transport) {
      ...
      if (destiny === TransportPoint.Pif || destiny === TransportPoint.Scanner) {
        ...
        url = `arrive/plate-number/${this.selectedLicensePlate}/${this.transport.transporteNi}`;
      } else {
