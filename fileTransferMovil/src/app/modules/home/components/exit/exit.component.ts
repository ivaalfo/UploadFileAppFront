...
export class ExitComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  protected onConfirmed(): void {
    ...
    if (this.selectedLicensePlate && this.transport) {
      this.actionSubscription = this.actionService.pifScannerExit(this.transport)
        .pipe(
          take(1))
        .subscribe(response => {
          if (response) {
  ...
  private goToNext(): void {
    ...
    if (this.selectedLicensePlate && this.transport) {
      ...
      if (destiny === TransportPoint.Terminal) {
        url = `returnTerminal/plate-number/${this.selectedLicensePlate}/${this.transport.transporteNi}`;
